...
def check_if_db_exists(db_name,db_user):
    ...
    command="/usr/bin/mysqlshow  -u "+db_user
...
def main(config,database):
    #try:
        ...
        if(config["DEFAULT"]["interactive"]=="yes"):
            mysql_dump_cmd = "/usr/bin/mysqldump --user="+config["DEFAULT"]["db_user"]+" -p "
        else:
            mysql_dump_cmd = "/usr/bin/mysqldump --user="+config["DEFAULT"]["db_user"]+ " "
